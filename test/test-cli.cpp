...
TEST_F(test_cli, file_with_syntax_errors_fails) {
  ...
  run_program_result r =
      run_program({get_quick_lint_js_executable_path(), test_file});
  EXPECT_EQ(r.exit_status, 1);
  EXPECT_THAT(to_string_view(r.output.string_view()),
              ::testing::HasSubstr("E0017"));
}
...
TEST_F(test_cli, file_with_escaped_syntax_errors_fails) {
  ...
  EXPECT_EQ(r.exit_status, 1);
  EXPECT_THAT(
      to_string_view(r.output.string_view()),
      ::testing::HasSubstr("[\u001b]8;;https://quick-lint-js.com/errors/E0017/"
                           "\u001b\\E0017\u001b]8;;\u001b\\]"));
}
...
TEST_F(test_cli, config_file_parse_error_prevents_lint) {
  ...
  EXPECT_THAT(to_string_view(r.output.string_view()),
              ::testing::Not(::testing::HasSubstr("myGlobalVariable")))
      << "test.js shouldn't be linted";
  EXPECT_THAT(to_string_view(r.output.string_view()),
              ::testing::Not(::testing::HasSubstr("E0057")))
      << "test.js shouldn't be linted";

  EXPECT_THAT(to_string_view(r.output.string_view()),
              ::testing::HasSubstr("quick-lint-js.config"))
      << "quick-lint-js.config should have errors";
  EXPECT_THAT(to_string_view(r.output.string_view()),
              ::testing::HasSubstr("E0164"))
      << "quick-lint-js.config should have errors";
}
...
TEST_F(test_cli, errors_for_all_config_files_are_printed) {
  ...
  EXPECT_EQ(r.exit_status, 1);
  EXPECT_THAT(to_string_view(r.output.string_view()),
              ::testing::HasSubstr("dir1"));
  EXPECT_THAT(to_string_view(r.output.string_view()),
              ::testing::HasSubstr("dir2"));
...
TEST_F(test_cli, auto_hyperlinks_are_not_printed_in_dumb_terminals) {
  ...
  if (pid == 0) {
    ...
    int exit_status = wait_for_process_exit(pid);

    EXPECT_EQ(exit_status, 1);
    EXPECT_THAT(to_string_view(output->string_view()),
                ::testing::Not(::testing::HasSubstr("\x1b")));
  }
}
#endif
}
